...
# Delete root qdisc from eth0 in case that we execute this script for the second time
...
tc class add dev eth0 parent 1:0 classid 1:2 htb rate $RATE ceil $RATE
...
tc qdisc add dev eth0 parent 1:2 tbf rate $RATE latency 50ms burst $RATE
# Route packets marked with 0x8 to 1:3 qdisc which is for development server
tc filter add dev eth0 parent 1:0 protocol ip prio 1 handle 8 fw flowid 1:2
# Show result
